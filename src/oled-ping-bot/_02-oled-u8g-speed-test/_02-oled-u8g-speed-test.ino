...
// We are using the 128 byte 4W Hardware SPI with no rotation landscape which only uses 27% of dynamic memory
...
void loop(void) {
  ...
  do {    
    ...
    int box_vertical_lines = round(counter/128) % 48;
    int horizontal =  counter % 128;
    if (box_vertical_lines > 0) u8g2.drawBox(0, 9, 128, box_vertical_lines); // x, y, w, h
    u8g2.drawHLine(0, box_vertical_lines + 9, horizontal);
    u8g2.setCursor(0,63);
    u8g2.print(counter);
  } while ( u8g2.nextPage() );
  // delay(10);
